function bursts=extract_bursts_trial(raw_trial, TF, times, search_freqs, band_lims, fooof_thresh, sfreq, varargin)
...
% TF = trials x freq x time
    ...
    if all(trial_TF(:)==0)
        disp('All values equal 0 after Fooof subtraction');
        return;
    end
    ...
    while true
        ...
        if peak_freq>=band_lims(1) && peak_freq<=band_lims(2) && ~hv_isnan
            ...
            dur = [max([1, peak_time_idx - left_loc]),...
                min([length(raw_trial), peak_time_idx + right_loc])];
